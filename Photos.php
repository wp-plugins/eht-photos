...
[photos images={1} thumbs={2} path={3}]

Where:
   {1} this is the URL of the path to the photo files
   {2} this is the path to the photo files in the file system
   {3} this is a flag to show (yes) or not (no) the path links

Example:

[photos images=/galeria/fotos thumbs=/galeria/miniaturas path=yes]
...
define ("EHT_PHOTOS_PLUGIN_VERSION", "0.7");
...
define ("EHT_PHOTOS_TABLE_PHOTO_INSERT",
		"INSERT INTO " . EHT_PHOTOS_TABLE_PHOTO . " (md5, name, views) VALUES (\"%s\", \"%s\", 0);");
...
{
	$search = "/\[photos\s*images\s*=\s*([^\]]+)\s*thumbs\s*=\s*([^\]]+)\s*path\s*=\s*([^\]]+)\s*\]/i";
	...
	if (is_array ($results))
	{
		...
		{
			$tagImages = trim ($results[1][$index]);
			$tagThumbs = trim ($results[2][$index]);
			$tagPath = (strcasecmp (trim ($results[3][$index]), EHT_PHOTOS_YES) == 0);
			if ((strlen ($tagImages) > 0) && (strlen ($tagThumbs) > 0))
			{
				$goodUrl = get_option ("siteurl");
				if (strlen ($goodUrl) > 0)
				{
					if ($goodUrl[strlen ($goodUrl) - 1] == EHT_PHOTOS_SLASH)
					{
						$goodUrl = substr ($goodUrl, 0, strlen ($goodUrl) - 1);
					}
				}
				
				if ($tagImages[0] != EHT_PHOTOS_SLASH)
				{
					$tagImages = EHT_PHOTOS_SLASH . $tagImages;
				}
				if ($tagThumbs[0] != EHT_PHOTOS_SLASH)
				{
					$tagThumbs = EHT_PHOTOS_SLASH . $tagThumbs;
				}
				if ($tagImages[strlen ($tagImages) - 1] != EHT_PHOTOS_SLASH)
				{
					$tagImages .= EHT_PHOTOS_SLASH;
				}
				if ($tagThumbs[strlen ($tagThumbs) - 1] != EHT_PHOTOS_SLASH)
				{
					$tagThumbs .= EHT_PHOTOS_SLASH;
				}
				
				$urlImages = $goodUrl . $tagImages;
				$pathImages = $_SERVER["DOCUMENT_ROOT"] . $tagImages;
				$urlThumbs = $goodUrl . $tagThumbs;
				$pathThumbs = $_SERVER["DOCUMENT_ROOT"] . $tagThumbs;
							   ...
							   $index)
{
	...
	$md5 = md5_file ($pathImages . $currentPath . $name);
	$description = EHTPhotosGetDescription ($md5, false, $name);
	if ($description != "")
	{
		$text .= "               $description\n";
	}
							  ...
							  $limit)
{
	...
	if ($type == EHT_PHOTOS_THUMB_FILE)
	{
		$thumb = EHTPhotosGetThumb ($urlImages . $currentPath,
									...
									$thumbSize);
		$md5 = md5_file ($pathImages . $currentPath . $name);
		$description = EHTPhotosGetDescription ($md5, true, $name);
	}
	else if ($type == EHT_PHOTOS_THUMB_FOLDER)
	{
								  ...
								  $photo)
{
	...
	{
		$sql = sprintf (EHT_PHOTOS_TABLE_PHOTO_INSERT, $md5, $photo);
		if (!($wpdb->query ($sql)))
		{
			$text .= "Fail to create: '$sql'<br>\n";
		}
		else
		{
			$ok = true;
	...
	if ($ok && (!$short) && ($id != 0))
	{
		$sql = sprintf (EHT_PHOTOS_TABLE_PHOTO_UPDATE_VIEWS, $views + 1, $id);
		if (!($wpdb->query ($sql)))
		{
			$text .= "Fail to update: '$sql'<br>\n";
			$ok = false;
		}
		else
		{
...
{
	$action = $_POST[EHT_PHOTOS_FIELD_ACTION];
	if ($action == EHT_PHOTOS_ACTION_UPDATE)
	{
		$optionThumb = $_POST[EHT_PHOTOS_OPTION_THUMB];
		...
		$optionNormal = $_POST[EHT_PHOTOS_OPTION_NORMAL];
		...
		$optionWidth = $_POST[EHT_PHOTOS_OPTION_WIDTH];
		...
		$optionExif = $_POST[EHT_PHOTOS_OPTION_EXIF];
	}
	else
	{
		$optionThumb = get_option (EHT_PHOTOS_OPTION_THUMB);
	...
	echo "</form>\n";
	echo "</div>\n";
	echo "<p align=\"center\">" . EHT_PHOTOS_PLUGIN_DESCRIPTION . "</p>\n";
}
